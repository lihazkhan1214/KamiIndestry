...
function Page() {
  ...
  if (isClient && session.data?.user?.role !== "admin") { // Check isClient before accessing location
                  ...
                  {displayedItems.map((item) => (
                    <tr key={item._id} className="border-b-2 border-[#D3D3D3]  hover:bg-gray-100">
                      ...
                      <td className="text-center text-sm  text-[#333237]">
                        <EditButtton id={item._id}/>
                        <DeleteIcon onClick={() => handleDelete(item._id)} className='text-[#FF3103] cursor-pointer mx-2' />
                      </td>
                    </tr>
                  ))}
                </tbody>
